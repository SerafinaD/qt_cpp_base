...
myfilemanager::myfilemanager(QWidget *parent) : QMainWindow(parent), model(nullptr)
{
...
void myfilemanager::chgDisk(int index)
{
   QFileInfoList list = QDir::drives();
   rebuildModel(list.at(index).path());
}

void myfilemanager::goMainPath()
{
   rebuildModel("/");
}

void myfilemanager::setNewModel(QStandardItemModel *newmodel)
{
    tree->setModel(newmodel);
    model = newmodel;
}

void myfilemanager::rebuildModel(QString str)
{
   curretnPath = str;
